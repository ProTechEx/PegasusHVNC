	...
	public class FormBlockClients : Form
	{
		...
		private void FormBlockClients_Load(object sender, EventArgs e)
		{
			try
			{
				...
				if (string.IsNullOrWhiteSpace(global::PEGASUS.Properties.Settings.Default.txtBlocked))
				{
					return;
				}
				string[] array = global::PEGASUS.Properties.Settings.Default.txtBlocked.Split(',');
		...
		private void FormBlockClients_FormClosed(object sender, FormClosedEventArgs e)
		{
			try
			{
				lock (Settings.Blocked)
				{
					...
					global::PEGASUS.Properties.Settings.Default.txtBlocked = string.Join(",", list);
		...
		private void InitializeComponent()
		{
			...
			listBlocked.DataBindings.Add(new System.Windows.Forms.Binding("Name", Properties.Settings.Default, "txtBlocked", true, System.Windows.Forms.DataSourceUpdateMode.OnPropertyChanged));
			...
			listBlocked.Name = Properties.Settings.Default.txtBlocked;
