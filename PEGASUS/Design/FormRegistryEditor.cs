...
namespace PEGASUS.Design
{
	public class FormRegistryEditor : Form
	{
		...
		private void FrmRegistryEditor_Load(object sender, EventArgs e)
		{
			...
			if (!ParentClient.Admin)
			{
				label2.Text = "The client software is not running as administrator " + Environment.NewLine + "some functionality like Update, Create, " + Environment.NewLine + "Open and Delete may not work properly!" + Environment.NewLine;
			}
			else
			{
		...
		private void InitializeComponent()
		{
			...
			tvRegistryDirectory = new IcarusWings.RegistryTreeView();
			imageRegistryDirectoryList = new System.Windows.Forms.ImageList(components);
			lstRegistryValues = new IcarusWings.AeroListView();
